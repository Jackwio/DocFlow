...
public class DocFlowHttpApiHostModule : AbpModule
{
    ...
    private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<DocFlowDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}DocFlow.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<DocFlowDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}DocFlow.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<DocFlowApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}DocFlow.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<DocFlowApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}DocFlow.Application"));
            });
        }
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "DocFlow API");

            var configuration = context.ServiceProvider.GetRequiredService<IConfiguration>();
            c.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
            c.OAuthScopes("DocFlow");
        });
